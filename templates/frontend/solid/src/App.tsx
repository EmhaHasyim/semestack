...
export default function App() {
	...
	const [greetingMessage, setGreetingMessage] = createSignal<string | null>(null);
	const [greetingRuntime, setGreetingRuntime] = createSignal<string | null>(null);
	const [greetingLoading, setGreetingLoading] = createSignal(false);

	const [pingStatus, setPingStatus] = createSignal<"idle" | "success" | "error">("idle");
	const [pingLatency, setPingLatency] = createSignal<number | null>(null);
	const [pingLoading, setPingLoading] = createSignal(false);
	const [systemTime, setSystemTime] = createSignal<string | null>(null);
	...
	async function sendGreeting(e: Event) {
		...
		try {
			...
			setGreetingMessage("Could not communicate with Elysia edge server.");
			setGreetingRuntime(null);
		} finally {
		...
		<main class="relative min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-violet-500/30 selection:text-violet-200 antialiased overflow-hidden flex flex-col justify-between">
			...
			<header class="relative z-10 w-full max-w-7xl mx-auto px-6 py-6 flex items-center justify-between border-b border-zinc-900">
				<div class="flex items-center gap-3">
					...
					<span class="text-sm font-semibold tracking-wider uppercase text-zinc-400">
						Semestack
					</span>
				</div>
				...
				<div class="grid md:grid-cols-2 gap-6 w-full">
					<div class="flex flex-col justify-between bg-zinc-900/30 backdrop-blur-md border border-zinc-900 hover:border-zinc-800/80 rounded-2xl p-6 transition-all duration-300">
						<div class="flex flex-col gap-4">
							...
							<div class="flex flex-col items-center justify-center bg-zinc-950/80 border border-zinc-900 rounded-xl py-8 gap-2 relative overflow-hidden group min-h-27.5">
								<Show
									when={!pingLoading()}
									fallback={
								...
								>
									<Show when={pingStatus() === "success"}>
										<div class="text-center transition-all duration-300 starting:opacity-0 opacity-100">
											<div class="text-4xl font-black text-emerald-400 tracking-tight font-mono">
												{pingLatency()}
												<span class="text-lg font-medium ml-1">ms</span>
											</div>
									...
									<Show when={pingStatus() === "error"}>
										<div class="text-center text-red-400 transition-all duration-300 starting:opacity-0 opacity-100">
											<span class="text-sm font-semibold font-mono">Connection Failed</span>
										</div>
									</Show>
									<Show when={pingStatus() === "idle"}>
										<div class="text-center text-zinc-600 transition-all duration-300 starting:opacity-0 opacity-100">
							...
							<div class="min-h-19 flex items-center justify-center bg-zinc-950/80 border border-zinc-900 rounded-xl p-4 relative overflow-hidden">
								...
								>
									<Show
										...
										fallback={
											<span class="text-xs text-zinc-600 uppercase tracking-widest font-mono transition-all duration-300 starting:opacity-0 opacity-100">
												No response received
											</span>
										}
									>
										<div class="w-full text-center transition-all duration-300 starting:opacity-0 starting:translate-y-2 opacity-100 translate-y-0">
