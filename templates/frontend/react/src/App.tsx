	...
	return (
		<div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-500/30 selection:text-blue-900 flex flex-col relative z-0">
			<div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
				...
				<div
					className="absolute top-[10%] right-[-10%] w-[50%] h-[50%] bg-fuchsia-500/30 rounded-full blur-[120px] animate-pulse"
					style={{ animationDelay: "1s" }}
				></div>
				<div
					className="absolute bottom-[-10%] left-[20%] w-[60%] h-[60%] bg-emerald-500/20 rounded-full blur-[120px] animate-pulse"
			...
			<header className="relative z-10 w-full bg-white/30 backdrop-blur-xl border-b border-white/50 shadow-[0_4px_30px_rgba(0,0,0,0.05)]">
				<div className="max-w-7xl mx-auto px-6 lg:px-8 py-4 flex items-center justify-between">
					<div className="flex items-center gap-3">
						...
						<span className="text-lg font-bold tracking-tight text-slate-800 hidden sm:block drop-shadow-sm">
							Semestack
						</span>
					</div>
			...
			<main className="relative z-10 flex-1 w-full max-w-7xl mx-auto px-6 lg:px-8 py-16 md:py-24 flex flex-col items-center gap-16">
				<div className="flex flex-col items-center text-center gap-6 max-w-3xl w-full">
					...
					<h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-slate-800 drop-shadow-sm">
						{stack[0]} <span className="text-blue-500">+</span> {stack[1]}{" "}
						<span className="text-blue-500">+</span> {stack[3]}
					</h1>
						...
						<div className="flex-1 p-6 flex flex-col items-center justify-center min-h-[140px]">
							{pingLoading ? (
								<div className="flex flex-col items-center starting:opacity-0 opacity-100 transition-opacity duration-500">
									...
									<span className="text-[10px] font-bold tracking-widest text-slate-500 uppercase animate-pulse">
										Contacting Edge...
									</span>
								</div>
							) : pingStatus === "success" ? (
								<div className="flex flex-col items-center starting:opacity-0 starting:scale-95 opacity-100 scale-100 transition-all duration-300">
									...
									<span className="text-[11px] text-slate-600 font-mono mt-2 bg-white/50 shadow-sm px-2 py-0.5 rounded-md border border-white/50">
										{systemTime}
									</span>
								</div>
							) : pingStatus === "error" ? (
								<span className="text-red-600 text-sm font-bold starting:opacity-0 opacity-100 transition-opacity duration-300 drop-shadow-sm">
									Connection Failed
								</span>
							) : (
								<span className="text-slate-500 font-mono text-sm font-medium starting:opacity-0 opacity-100 transition-opacity duration-300">
									Ready to ping
								</span>
							)}
						</div>
						...
						<div className="flex-1 p-6 flex flex-col items-center justify-center min-h-[140px]">
							{greetingLoading ? (
								<div className="flex flex-col items-center gap-3 starting:opacity-0 opacity-100 transition-opacity duration-500">
									<svg
										className="size-8 animate-spin text-blue-600 drop-shadow-md"
									...
									<span className="text-xs font-bold text-slate-500 animate-pulse tracking-wide uppercase">
										Processing...
									</span>
								</div>
							) : greetingMessage ? (
